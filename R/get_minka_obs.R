...
#'
#'   ## Filter by a bounding box of Barcelona
#'   bounds <- c(38.44047, -125, 40.86652, -121.837)
#'   get_minka_obs(query = "Salpa salpa", bounds = bounds)
#'
#'   ## Filter by taxon, allows higher order filtering,
#'   ## Here we can search for just stone flies (order Molusca)
...
get_minka_obs <- function(query = NULL, taxon_name = NULL, taxon_id = NULL,
  ...
  arg_vals <- lapply(arg_list, is.null)
                      ...
                      if (all(unlist(arg_vals))) {
                        stop("Tots els parametres de la cerca no poden ser NULL. Indica almenys un.")
                      }
  search <- ""
                      if(!is.null(query)){
